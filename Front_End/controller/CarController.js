...
function bindTrEvents() {
    ...
    for (let i = 0; i < rows; i++) {
        $("#carTable").children().eq(i).children(":eq(0)").click(function () {
        ...
        success: function (resp) {
            ...
            for(let car of resp.data){
                let row =`<tr>
                            ...
                            <td>yes</td>
                            <td>
                             <button type="button" class="btn btn-outline-light btnRemove">Remove</button>                       
                            </td>
                            <td>   
                            <button type="button" class="btn btn-outline-light btnUpdate">Update</button>
                            </td>
                </tr>`;
                $("#carTable").append(row);
            }
...
function deleteCars() {
    let rows = $("#carTable").children().length;
    for (let i = 0; i < rows; i++) {
        $("#carTable").children().eq(i).children(":eq(6)").click(function () {
            let registration_Number = $("#registrationNumber").val();
            $.ajax({
                url: "http://localhost:8080/Back_End_war/car?registration_Number=" + registration_Number,
                method: "delete",
                success: function (resp) {
...
function updateCars() {
    let rows = $("#carTable").children().length;
    for (let i = 0; i < rows; i++) {
        $("#carTable").children().eq(i).children(":eq(7)").click(function () {
            setCarType();
            setCarTransmissionType();
            let registrationNumber = $("#registrationNumber").val();
            let brand = $("#brand").val();
